function [x,converged,errf_k,errDeltax_k,relDeltax_k,iterations,X] = NewtonNd_homotopy(eval_f,x0,p,u,errf,errDeltax,relDeltax,MaxIter,visualize,FiniteDifference,eval_Jf,q)
...
while k<=MaxIter & (errf_k>errf | errDeltax_k>errDeltax | relDeltax_k>relDeltax),
   ...
   if FiniteDifference
      Jf = eval_Jf_FiniteDifference_DTFE(eval_f,X(:,k),p,u,q);
   else 
      Jf = feval(eval_Jf,X(:,k),p,u,q);
   end
